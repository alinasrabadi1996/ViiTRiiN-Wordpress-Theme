...
<table id="collapse-table" class="table-order">
    ...
    $args = array(
        ...
        'meta_query' => array(
            ...
            array(
                'key' => '_billing_phone',
                'value' => "0".$_SESSION['verfied_mobile'],
                'compare' => '='
            ),
            array(
                'key' => '_billing_phone',
                'value' => substr($_SESSION['verfied_mobile'], 1),
                'compare' => '='
            )
        )
    );
    ...
    foreach ( $orders as $key => $value ) {
        ...
        switch($order->get_status()) {
            ...
            case "pending" :
                ...
                break;
            case "sales" :
                $product_status = "تایید واحد فروش";
                $status_label = "label-primary";
                break;
            case "depot" :
                $product_status = "تایید واحد انبار";
                $status_label = "label-info";
                break;
            case "sending" :
                $product_status = "تایید واحد ارسال";
                $status_label = "label-megna";
                break;
            default :
            ...
            <td colspan="8">
                <div class="order-items">
                    <table class="table-order">
                        ...
                        foreach ($order->get_items() as $item_key => $item_values):
                            ...
                            <tr>
                                ...
                                <td><?php echo wc_price($line_subtotal / $product_quantity); ?></td>
        			...
        			$url = wp_nonce_url( add_query_arg( array(
        				'action' => $action,
        				'order_id' => $order_id
        			), site_url()."/invoice" ), $action, 'nonce' );
        			
        			$url = apply_filters( 'bewpi_pdf_packing_slip_url', $url, $order_id, $action );
        			printf( '<div class="invoice-button"><a href="%1$s" title="%2$s" class="button shop-order-action" target="_blank">%2$s</a></div>', $url, __( 'مشاهده فاکتور', 'woocommerce-pdf-invoices' ) );
                    ?>
                </div>
            </td>
        </tr>
